...
print(2 ** 2 ** 3)  # 2 ** (2 ** 3) = 2 ** 8 ответ 256

print(10 // 3)  # округление в меньшую сторону 3
print(-10 // 3)  # округление в меньшую сторону -4

# Остаток от деления a % b можно найти по формуле: a % b = a - b * (a // b)
# Где a // b — это целая часть от деления.
print(10 % 3)
print(10 % 4)
print(10 % 5)
print(10 % 6)
# Когда делитель больше делимого, остаток равен самому делимому.
# 10 ÷ 12 = 0 (целая часть, потому что 12 > 10)
# 12 × 0 = 0 (ничего не "ушло")
# 10 - 0 = 10 (остаток)
print(10 % 12)
print(10 % 20)
...
num = 754
a = num % 10  # вычисляем последнюю цифру числа
b = (num % 10) // 10  # вычисляем среднюю цифру числа
...
# для вычисления i-й цифры n-значного числа num в общем виде: (num // 10 ** (n - i)) % 10
# другой алгоритм получения каждой цифры n-значного числа num: num % 10 ** (n + 1 - i) // (10 ** (n - i))
